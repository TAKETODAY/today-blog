...
export function handleValidationError(err, validationCallback, other) {
  const { data, response } = err
  if (response && response.status === 400 && data.validation) {
    const { validation } = data
    message.error(validation[0].message);
    validationCallback && validationCallback(data.validation)
  }
  else {
    other && other(err)
  }
