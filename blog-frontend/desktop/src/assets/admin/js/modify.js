...
$(document).ready(function(){
	...
	//	console.log(currentArticleId)
		...
		let article_modify = getStorage("article_modify")
		if(article_modify != null && article_modify.id == currentArticleId) {
			...
			ajax("GET", contextPath + "/api/articles/"+ currentArticleId ,null,null,function(data) {
				if(data.success) {
					...
					saveStorage("article_modify", Article)
					full()
				} else {
					if(data.code == 401){
						$.toast("登录超时","cancel")
...
function savePassword(text) {
	...
	chooseCategory(function() {
		...
		Article.content = UE.getEditor('editor').getContent()//getContentTxt()
