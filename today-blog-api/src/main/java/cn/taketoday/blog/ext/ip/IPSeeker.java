...
public final class IPSeeker {
  ...
  private static final Map<String, IPLocation> IP_CACHE = new ConcurrentHashMap<>();
  // 随机文件访问类
  private static final RandomAccessFile IP_FILE;
  // 内存映射文件
  private MappedByteBuffer mbb;
  // 起始地区的开始和结束的绝对偏移
  private static final long IP_BEGIN, IP_END;
  // private byte[] buf;
  private final byte[] b4 = new byte[4];

  static {
    // FIXME 暂时不可用
    File file = new File(Objects.requireNonNull(ClassUtils.getDefaultClassLoader().getResource("qqwry.dat")).getFile());
    try {
      IP_FILE = new RandomAccessFile(file, "r");
      IP_BEGIN = readLong4(0);
      IP_END = readLong4(4);
      if (IP_BEGIN == -1 || IP_END == -1) {
        IP_FILE.close();
        throw new IllegalStateException("IP file format error!");
      }
    }
    catch (IOException e) {
      throw new IllegalStateException("IP file read error!");
    }
  }
  ...
  public List<IPEntry> getIPEntriesDebug(String s) {
    ...
    long endOffset = IP_END + 4;
    for (long offset = IP_BEGIN + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
      // 读取结束IP偏移
      ...
      if (temp != -1) {
        ...
        if (loc.getCountry().contains(s) || loc.getArea().contains(s)) {
  ...
  public List<IPEntry> getIPEntries(String s) {
    ...
    try {
      // 映射IP信息文件到内存中
      if (mbb == null) {
        FileChannel fc = IP_FILE.getChannel();
        mbb = fc.map(FileChannel.MapMode.READ_ONLY, 0, IP_FILE.length());
        mbb.order(ByteOrder.LITTLE_ENDIAN);
      }
      int endOffset = (int) IP_END;
      for (int offset = (int) IP_BEGIN + 4; offset <= endOffset; offset += IP_RECORD_LENGTH) {
  ...
  public String getCountry(byte[] ip) {
    ...
    final IPLocation ipLocation = IP_CACHE.get(ipStr);
    if (ipLocation != null) {
      return ipLocation.getCountry();
    }
    IPLocation loc = getIPLocation(ip);
    IP_CACHE.put(ipStr, loc.getCopy());
  ...
  public String getArea(byte[] ip) {
    ...
    String ipStr = IPUtils.getIpStringFromBytes(ip);
    // 先检查cache中是否已经包含有这个ip的结果，没有再搜索文件
    IPLocation loc = IP_CACHE.get(ipStr);
    if (loc == null) {
  ...
  private IPLocation getIPLocation(byte[] ip) {
    ...
    if (info == null) {
      info = new IPLocation(IPLocation.UNKNOWN, IPLocation.UNKNOWN);
    }
    return info;
  }
  ...
  private static long readLong4(long offset) {
    ...
    try {
      IP_FILE.seek(offset);
      ret |= (IP_FILE.readByte() & 0xFF);
      ret |= ((IP_FILE.readByte() << 8) & 0xFF00);
      ret |= ((IP_FILE.readByte() << 16) & 0xFF0000);
      ret |= ((IP_FILE.readByte() << 24) & 0xFF000000);
      return ret;
    }
    catch (IOException e) {
      return -1;
  ...
  private long locateIP(byte[] ip) {
    ...
    if (r == 0)
      ...
      return -1;
    // 开始二分搜索
    for (long i = IP_BEGIN, j = IP_END; i < j; ) {
  ...
  private IPLocation getIPLocation(long offset) {
    try {
      ...
      // 跳过4字节ip
      IP_FILE.seek(offset + 4);
      // 读取第一个字节判断是否标志字节
      byte b = IP_FILE.readByte();
      if (b == AREA_FOLLOWED) {
        // 读取国家偏移
        long countryOffset = readLong3();
        // 跳转至偏移处
        IP_FILE.seek(countryOffset);
        // 再检查一次标志字节，因为这个时候这个地方仍然可能是个重定向
        b = IP_FILE.readByte();
        if (b == NO_AREA) {
          country = readString(readLong3());
          IP_FILE.seek(countryOffset + 4);
        }
        else {
          country = readString(countryOffset);
        }
        area = readArea(IP_FILE.getFilePointer());
        // 读取地区标志
      }
      else if (b == NO_AREA) {
        country = readString(readLong3());
        ...
        country = readString(IP_FILE.getFilePointer() - 1);
        area = readArea(IP_FILE.getFilePointer());
      }
  ...
  private String readString(long offset) {
    try {
      ...
      while ((buf[i] = IP_FILE.readByte()) != 0) {
        ++i;
        if (i >= buf.length) {
  ...
  public static void main(String[] args) {
    ...
    String ip = "175.152.185.76";
    //		String ip = "202.101.224.69";

    System.out.println(ipSeeker.getAddress(ip));
